...
var token = 'token',
    ...
    drawn = false,
    type = '2',
    prize_msg = '恭喜您获得1.00元微信现金红包, 将通过公众号发放, 请尽快领奖!';
...
$('#start-btn').bind('click', function () {
    $('#start-btn').remove();
    $('#message button').after('<i class="fa fa-spinner fa-spin fa-fw"></i>');
    if (!navigator.geolocation) {
        $('#message button').after('浏览器不支持获取位置, 无法参与抽奖!');
    }
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (0 == data['code']) {
                    ...
                    if (drawn) {
                        if (received) {
                            $('#message button').after('您已兑奖成功, 感谢您的支持!');
                        }
                        else {
                            if ('0' == type) {
                                $('#message button').after('您已抽奖, 很遗憾您没有中奖, 感谢您的支持!')
                            }
                            else {
...
$(function () {
    $('#lottery-start').bind('click', function () {
        ...
        $('.prizes').lottery({
            winId: Math.ceil(prizeInfo[type]['winId'] - 1), //中奖序号
            round: 3, //跑的基础轮次
            time: 5, //动画时间
            activeClass: 'active', //中奖高亮样式
            effect: 'Cubic', //动画曲线
            finish: function () {
                if ('0' == type) {
